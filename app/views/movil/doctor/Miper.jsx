...
import searchNodeByUid from '../../../../src/queryfb/doctor/searchNodeByUid'; // Módulo para buscar el doctor por UID
        ...
        .then(data => {
          if (data) {
            setDoctorData({
              ...
              identificacion: data.dui || 'No disponible',
              nacionalidad: data.nacionalidad || 'No disponible',
              telefono: data.telefono || 'No disponible',
              email: data.email || 'No disponible',
              jvpm: data.jvpm || 'No disponible',
            });
          }
        })
        .catch(error => console.error("Error al cargar datos del doctor:", error));
    }
  }, [currentUser]);
    ...
    <ScrollView style={styles.scrollView}>
      <View style={styles.container}>
        <View style={styles.header}>
          ...
          <FontAwesome name="user-md" size={80} color="#45B5A9" style={styles.profileIcon} />
        </View>

        <View style={styles.infoSection}>
          <Text style={styles.sectionTitle}>Información básica</Text>

          <InfoRow icon="map-marker" label="Dirección" value={doctorData.direccion} />
          <InfoRow icon="id-card" label="Identificación" value={doctorData.identificacion} />
          <InfoRow icon="globe" label="Nacionalidad" value={doctorData.nacionalidad} />
          <InfoRow icon="phone" label="Teléfono" value={doctorData.telefono} />
          <InfoRow icon="envelope" label="Correo electrónico" value={doctorData.email} />
          <InfoRow icon="briefcase" label="Número de Registro (JVPM)" value={doctorData.jvpm} />
        </View>
...
const styles = StyleSheet.create({
  ...
  // Estilos para la modal y botones
  modalBackground: {
    flex: 1,
